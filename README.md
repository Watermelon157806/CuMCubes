...
The following example creates a CUDA `torch.Tensor` volume with spherical iso-surfaces and extracts one of them (i.e., a sphere):
```python
>>> import torch
>>> import cumcubes

>>> device = torch.device("cuda:0")
>>> X, Y, Z = torch.meshgrid(torch.linspace(0, 99, 200, device=device), torch.linspace(0, 99, 200, device=device), torch.linspace(0, 99, 200, device=device), indexing="ij")
...
<img src="asserts/bunny_cumcubes.png" height="342"/>
The image is the result of `cumcubes.marching_cubes`. (The Stanford Bunny Example)
